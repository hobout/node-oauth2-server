...
var bootstrap = function (model, params, continueAfterResponse) {
  ...
  app.post('/authorise', app.oauth.authImplicitGrant(function (req, next) {
    next.apply(null, params || []);
  }));

  app.get('/authorise', app.oauth.authImplicitGrant(function (req, next) {
    next.apply(null, params || []);
  }));
...
describe('AuthImplicitGrant', function() {
  ...
  it('should accept valid request and return token using POST', function (done) {
    ...
    request(app)
      ...
      .expect(302, function (err, res) {
        res.header.location.should.equal('http://nightworld.com?access_token=' + token);
        done();
      });
  });
  ...
  it('should accept valid request and return token and state using GET', function (done) {
    var token;
    ...
    request(app)
      ...
      .expect(302, function (err, res) {
        res.header.location.should.equal('http://nightworld.com?access_token=' + token  + '&state=some_state');
        done();
      });
  });
